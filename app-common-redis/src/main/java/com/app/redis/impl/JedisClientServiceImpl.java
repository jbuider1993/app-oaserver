...
import com.app.redis.util.RedisToolUtil;
				...
				if(entry.getKey().indexOf(ip) != -1){
					...
					if (logList != null && !logList.isEmpty()) {
						...
						for (Slowlog sl : logList) {
							...
							flag = true;
							op.put("id", sl.getId());
							op.put("executeTime", RedisToolUtil.getDateStr(sl.getTimeStamp() * 1000));
		...
		try {
			...
			while (entries.hasNext()) {
				...
				if(entry.getKey().indexOf(ip) != -1){
					...
					map.put("createTime", RedisToolUtil.getTimeAndToString());
